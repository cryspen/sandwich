module Sandwich.Implementation.Openssl3.Error
    ...
    fun (err: u64) ->
      {
        ...
        Core.Result.impl__unwrap_or #t_ErrorLibrary
          ...
          (Core.Convert.f_try_into #i32
              ...
              (Sandwich.Implementation.Openssl3.Support.err_get_lib err <: i32)
            <:
            Core.Result.t_Result t_ErrorLibrary Prims.unit)
          (ErrorLibrary_None <: t_ErrorLibrary);
        f_reason = Sandwich.Implementation.Openssl3.Support.err_get_reason err
      }
      <:
      t_Error
  }
      ...
      | SslError_Ssl  ->
        let error:t_Error =
          Core.Convert.f_from #t_Error
            #u64
            #FStar.Tactics.Typeclasses.solve
            (Sandwich.Implementation.Openssl3.Support.peek_last_error () <: u64)
        in
        if
